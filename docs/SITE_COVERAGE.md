...
기본 수집 주기는 와이고수·뽐뿌·디시인사이드·인벤 5분, 아카라이브 10분,
더쿠 15분, 에펨코리아 30분이다. 각 사이트는 독립 작업으로 예약되므로 접근 제한이
강한 사이트의 주기를 늘려도 다른 사이트의 수집 신선도에는 영향을 주지 않는다.
현재 운영에서는 원문 HTTP 403/430 차단이 확인된 `fmkorea`, `theqoo`, `arca`를
`CRAWLER_DISABLED_SITES`로 임시 중지한다. 이 값에서 사이트를 제거하고 프로세스를
재시작하면 해당 수집기를 다시 활성화할 수 있다.
...
새 인기글 본문 요청은 아카라이브·더쿠에서 1초, 에펨코리아에서 3초 간격으로
처리한다. HTTP 429/430은 즉시 재시도하지 않으며 `Retry-After` 동안 사이트별
쿨다운을 적용한다. 쿨다운을 유발한 게시물은 저장하지 않아 다음 정상 주기에 다시
수집할 수 있다.
